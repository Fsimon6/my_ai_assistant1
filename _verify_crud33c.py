...
def register(u, pw):
    _req("POST", "/api/v1/auth/register", {"username": u, "email": u + "@x.com", "password": pw, "password_confirm": pw})
def login(u, pw):
...
def create(u, token):
    ...
    assert s == 200, ("create failed", s, d); return d.get("data") or d.get("character") or d
def listc(token):
    s, d = _req("GET", "/api/v1/characters/", token=token); return d.get("data") or d.get("characters") or d
...
s, d = update(cid, {"name": "e2e33_upd", "system_prompt": "updated prompt"}, tU); check(s == 200, "Update name only -> 200")
s, det = detail(cid, tU); check(det["name"] == "e2e33_upd", "Name persisted"); check(det.get("model") in (None, ""), "Model still default after name-only update")
...
conv_before = db.query(Conversation).filter(Conversation.character_id == int(did), Conversation.user_id == uid).count(); check(conv_before > 0, "Conversation created for D")
...
# 先以所有者身份删除 C，再验证删除后聊天 -> 404
s, d = deletec(cid, tU); check(s == 200, "Delete C -> 200")
chunks, st = stream_chat(cid, tU, "hi"); check(st == 404, "Deleted C chat -> 404")

req = urllib.request.Request(BASE + "/api/v1/rag/query?stream=true", data=json.dumps({"query": "用一句话介绍AI。", "stream": True, "context_count": 3}).encode(), method="POST")
...
try:
    ...
    for line in r:
        ...
        if not line: continue
        o = json.loads(line)
        if o.get("type") == "chunk": rag_chunks.append(o["content"])
